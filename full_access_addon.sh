...
# This script creates a full access addon for TeslaBle2Mqtt and TeslaBleHttpProxy by copying the
...
# Remove the full access addon if it already exists
rm -rf TeslaBleHttpProxyFullAccess

# Copy the non-full access addon
cp -r TeslaBleHttpProxy TeslaBleHttpProxyFullAccess

# Patch the version, name, description, and slug
sed -i 's/\(version: "\([^"]*\)\)"/\1-fa"/g' TeslaBleHttpProxyFullAccess/config.yaml
sed -i 's/\(name: "\([^"]*\)\)"/\1 [Full access]"/g' TeslaBleHttpProxyFullAccess/config.yaml
sed -i 's/\(description: "\([^"]*\)\)"/\1 [Full access version]"/g' TeslaBleHttpProxyFullAccess/config.yaml
sed -i 's/\(slug: "\([^"]*\)\)"/\1-full-access"/g' TeslaBleHttpProxyFullAccess/config.yaml
# Add additional options
...
cat <<EOF >> TeslaBleHttpProxyFullAccess/README.md
